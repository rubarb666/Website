...
class RhubarbParsedown extends \ParsedownExtra
{
    ...
    function text($text)
    {
        while(preg_match('/[[]packagist:([^]]+)[]]/', $text, $match)){
            $package = $match[1];
            $packageEncoded = str_replace("/", "%2F", $package);
            $packageEncoded = str_replace("-", "--", $packageEncoded);

            $text = str_replace($match[0], '<div style="float:right">[![Package](https://img.shields.io/badge/packagist-'.$packageEncoded.'-ff69b4.svg)](https://packagist.org/packages/'.$package.')
[![Latest Stable Version](https://poser.pugx.org/'.$package.'/v/stable)](https://packagist.org/packages/'.$package.')</div>', $text);
        }

        return parent::text($text); // TODO: Change the autogenerated stub
    }
    ...
    public static function getHtmlForDemo($demoPath, $request = null)
    {
        if (file_exists($demoPath)) {
            ...
            $folderPath = dirname($demoPath);

            while($folderPath != ""){
                $folderPath = realpath($folderPath."/../");

                if (file_exists($folderPath."/vendor/autoload.php")){
                    include_once $folderPath."/vendor/autoload.php";
                    break;
                }
            }
